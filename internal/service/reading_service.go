...
func (s *ReadingService) List(ctx context.Context) ([]domain.Reading, error) {
	...
	for i := range items {
		items[i].Reflection = normalizeReflection(items[i])
	}
	return items, nil
}
...
func (s *ReadingService) SaveReflection(ctx context.Context, id string, in ReflectionInput) (domain.Reading, error) {
	if in.Rating < 0 || in.Rating > 5 {
		return domain.Reading{}, errors.New("rating must be between 0 and 5")
	}
	item, err := s.repo.Get(ctx, id)
	if err != nil {
		return domain.Reading{}, err
	...
	if !state.Eligible {
		return domain.Reading{}, fmt.Errorf("reflection available after %s", state.EligibleAt.Format(time.RFC3339))
	}
	now := time.Now().UTC()
	if err := s.repo.SaveReflection(ctx, id, in.Rating, strings.TrimSpace(in.Note), now); err != nil {
		return domain.Reading{}, err
	}
...
func (s *ReadingService) RelevanceStats(ctx context.Context) (domain.RelevanceStats, error) {
	items, err := s.repo.List(ctx)
	if err != nil {
		return domain.RelevanceStats{}, err
	}
...
func normalizeReflection(r domain.Reading) domain.Reflection {
	...
	if r.Reflection.CreatedAt != nil {
		created = r.Reflection.CreatedAt
	}
...
func (s *ReadingService) buildInterpretation(question, lang string, primary, relating int, changing []int) domain.ReadingInterpretation {
	...
	if lang == "cs" {
		...
		if r, ok := s.cs[relating]; ok {
			result.Relating = mapCzech(relating, r)
		}
	} else {
		...
		if r, ok := s.en[relating]; ok {
			result.Relating = mapEnglish(relating, r)
		}
	}
...
func summaryFor(lang string) string {
	if lang == "en" {
		return "Interpretation combines the primary hexagram, changing lines, and the relating hexagram."
	}
...
func renderMarkdown(question string, i domain.ReadingInterpretation, changing []int) string {
	...
	if i.Language == "en" {
		b.WriteString("# I Ching reading\n\n## Question\n\n" + question + "\n\n")
	} else {
		b.WriteString("# I-ťing věštba\n\n## Otázka\n\n" + question + "\n\n")
	}
	if i.Language == "en" {
		b.WriteString("## Primary hexagram\n\n")
	} else {
		b.WriteString("## Primární hexagram\n\n")
	}
	...
	if len(i.ChangingLines) > 0 {
		if i.Language == "en" {
			b.WriteString("## Changing lines\n\n")
		} else {
			b.WriteString("## Proměnné čáry\n\n")
		}
		for _, line := range i.ChangingLines {
			if i.Language == "en" {
				fmt.Fprintf(&b, "### Line %d\n\n%s\n\n", line.Line, line.Text)
			} else {
				fmt.Fprintf(&b, "### Čára %d\n\n%s\n\n", line.Line, line.Text)
			}
		}
	}
	if i.Language == "en" {
		b.WriteString("## Relating hexagram\n\n")
	} else {
		b.WriteString("## Výsledný hexagram\n\n")
	}
