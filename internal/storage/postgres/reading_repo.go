...
VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb,$8,$9)`, reading.ID, reading.Question, string(reading.Method), reading.PrimaryNumber, reading.RelatingNumber, string(changingJSON), string(linesJSON), reading.Interpretation, reading.CreatedAt)
	return reading, err
}
...
func (r *ReadingRepository) List(ctx context.Context) ([]domain.Reading, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT id, question, method, primary_number, relating_number, changing_lines::text, lines::text, interpretation, created_at FROM readings ORDER BY created_at DESC`)
...
func (r *ReadingRepository) Get(ctx context.Context, id string) (domain.Reading, error) {
	if err := ensureSchema(ctx, r.db); err != nil { return domain.Reading{}, err }
	row := r.db.QueryRowContext(ctx, `SELECT id, question, method, primary_number, relating_number, changing_lines::text, lines::text, interpretation, created_at FROM readings WHERE id = $1`, id)
